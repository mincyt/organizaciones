...
.directive("solrFacetGroup", function() {
  return {
    ...
    transclude: true,
    templateUrl:"view/solr_facet_group.html",
...
.directive("solrSelected", function(){
  return {
    ...
    transclude: true,
    templateUrl: "view/solr_selected.html",
    require:"^solr",
...
.directive("solrSearch", function($location) {
  return {
    ...
    restrict: "E",
    templateUrl:"view/solr_search.html",
    require: "^solr",
    link: function( scope, element, attrs, ctrl){
...
.directive("solrFacet", function() {
  return {
    ...
    templateUrl:"view/solr_facet.html",
    link: function( scope, element, attrs, ctrl){
      ctrl.registerFacet(scope);

      var es5getprops = Object.getOwnPropertyNames;
      scope.isEmpty = function() {
        return angular.isUndefined(scope.results) && (es5getprops(scope.results).length === 0);
      };
    }
  }
})
...
.directive("solrFacetResult", function($location) {
  return {
    ...
    require: "^solr",
    templateUrl:"view/solr_facet_result.html",
    link: function( scope, element, attrs, ctrl){
  ...
  return {
    ...
    controller: function($scope, $http, $location) {
      ...
      that.search = function(query, rows){
    	...
    	$http.jsonp(that.solrUrl, {params: that.buildSearchParams(), cache:false})
